...
If your hosts do not have internet access, you must download remote SCAP resources and distribute them from {ProjectServer} to your hosts as local files.
...
For more information, see {ContentManagementDocURL}managing-custom-file-type-content-in-{project-context}[Managing {customfiletype} content in {Project}] in _{ContentManagementDocTitle}_.
+
...
----
+
For more information about running remote jobs, see {ManagingHostsDocURL}configuring-and-setting-up-remote-jobs[Configuring and setting up remote jobs] in _{ManagingHostsDocTitle}_.
...
* {ManagingHostsDocURL}downloading-files-to-a-host-from-a-custom-file-type-repository-by-using-web-ui[Downloading files to a host from a {customfiletype} repository by using {ProjectWebUI}]
endif::[]
